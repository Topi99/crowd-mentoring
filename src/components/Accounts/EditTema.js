...
class EditAEsp extends React.Component {
  constructor(props) {
    ...
    if(!props.new) {
      this.props.firebase.db.collection('temas').doc(this.props.match.params.uid).onSnapshot(doc => {
  ...
  submit = async e => {
    if(!this.props.new) {
      await this.props.firebase.db.collection('temas').doc(this.props.match.params.uid).set({
        ...this.state
      });
    } else {
      let ref = await this.props.firebase.db.collection('temas').doc();
      let uid = ref.id;
      ref.set({
        ...this.state,
        uid,
